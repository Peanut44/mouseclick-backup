// Mouseclick TV - Vercel Android/TV HLS playlist proxy v5 query fallback
// Path: api/hls.js
// URL: https://mouseclick-backup.vercel.app/api/hls?name=peanut
...
function cleanName(raw) {
  if (!raw) return "";
  return String(raw).replace(/\.m3u8$/i, "").trim().toLowerCase();
}
...
export default async function handler(req, res) {
  ...
  if (!id) return res.status(404).send(`channel not found: ${name || "empty"}`);
